...
_WS_SCRIPT="/opt/nemoclaw-blueprint/scripts/ws-proxy-fix.js"
if [ -f "$_WS_SCRIPT" ]; then
  _WS_FIX_SCRIPT="/tmp/nemoclaw-ws-proxy-fix.js"
  emit_sandbox_sourced_file "$_WS_FIX_SCRIPT" << 'WS_FIX_EOF'
"use strict";
// SPDX-FileCopyrightText: Copyright (c) 2026 NVIDIA CORPORATION & AFFILIATES. All rights reserved.
// SPDX-License-Identifier: Apache-2.0
//
...
# OTLPExporters do not support proxy environment variables, so we create a custom proxy locally
_OTLP_PROXY_SCRIPT="/tmp/nemoclaw-otlp-proxy.js"
emit_sandbox_sourced_file "$_OTLP_PROXY_SCRIPT" <<'OTLP_PROXY_EOF'
const http = require("http");
const PROXY = { host: "10.200.0.1", port: 3128 };
const TARGET = `#REPLACE_OTEL_ENDPOINT_SED_PLACEHOLDER#`; // Otel collector endpoint
...
const server = http.createServer((clientReq, clientRes) => {
  ...
  const opts = {
    ...
    headers: { ...clientReq.headers, host: `#REPLACE_OTEL_HOST_SED_PLACEHOLDER#` }
  };
...
PROXYEOF
  ...
  # Observability plugin preload for connect sessions — conditional on the file existing.
...
if [ "$(id -u)" -ne 0 ]; then
  ...
  [ -n "${DB_API_PID:-}" ] && SANDBOX_CHILD_PIDS+=("$DB_API_PID")
  [ -n "${OTLP_PROXY_PID:-}" ] && SANDBOX_CHILD_PIDS+=("$OTLP_PROXY_PID")
