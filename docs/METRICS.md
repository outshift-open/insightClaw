...
| openclaw.agent.downstream_context_sharing| Histogram | On llm_input| estimates how much of the caller agent full context (prompt, history, system…) is covered by the sub-agent prompt (based on heuristics) – high score means the caller shares a lot of its context with the sub-agent.
