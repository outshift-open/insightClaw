 ...
 *
 * Detection approach:
 *   Tool spans arriving for the same (runtimeSessionKey, agentSpanId) are grouped.
...
/** Pending tool spans for the current agent turn, keyed by runtime session key */
const pendingTools = new Map<string, ForkGroup>();

/** Completed fork groups waiting for a join, keyed by runtime session key */
...
/**
 * Clean up fork/join state for a runtime session.
 */
